...
sub display_points {
  ...
  if ($sth->rows) {
    ...
    my $total_points = get_total_points($dbh, $username);
    my $total_points_inuse = get_total_points_inuse($dbh, $username);
...
EOT
	...
	$style = 'points_even_row';
      ...
      my $avail = zone_highest_price($dbh, $username, $zone_name);
      my $high_bid = zone_highest_bid($dbh, $username, $zone_name);
...
sub get_total_points_inuse {
  ...
  my $sth = $dbh->prepare("select sum(bid) from bid_eq where bidder=? and status='bidding'");
...
sub get_zone_points_inuse {
  ...
  my $sth = $dbh->prepare("select percent from zones where name=?");
  $sth->execute($zone);
  ...
  $sth = $dbh->prepare("select ceiling(sum(bid) * ? / 100) from bid_eq where bidder=? and zone=? and status='bidding'");
  $sth->execute($percent,$username,$zone);
...
sub zone_highest_bid {
  ...
  my $total_points = get_total_points($dbh, $username);
  my $total_points_in_use = get_total_points_inuse($dbh, $username);
  my $total_points_avail = $total_points - $total_points_in_use;

  my $zone_points = get_zone_points($dbh, $username, $zone);
  my $zone_points_in_use = get_zone_points_inuse($dbh, $username, $zone);
...
sub all_zone_highest_bid {
  ...
  foreach $cur_zone (@zone_list) {
    ...
    my $sth = $dbh->prepare("select percent from zones where name=?");
    $sth->execute($cur_zone);
    my ($percent) = $sth->fetchrow_array;

    my $zone_points = get_zone_points($dbh, $username, $cur_zone);
    my $zone_points_in_use = get_zone_points_inuse($dbh, $username, $cur_zone);
    my $zone_points_avail = $zone_points - $zone_points_in_use;
...
sub get_all_zone_points {
  ...
  my $sth = $dbh->prepare("select zone,points from user_points_$username");
  $sth->execute;
  my $data = $sth->fetchall_arrayref;
